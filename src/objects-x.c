...
/*
  ...
  [[ If we're going to use the full font name above, we can't treat it as
  any other encoding than Qbinary, because the bitmap of character coverage
  which forms part of it gives no guarantees that it'll encode as anything
  else. 

  However, fontconfig returns foundry and name information in UTF-8. So the
  shortname above will better correspond to the font's name, because we can
  treat it as valid UTF-8, and non-ASCII characters will show up properly
  as Mule characters. ]]

  False.  The repertoire is represented in an ASCII-compatible way.  See
  fccharset.c (FcCharSetUnparse).  So we can use UTF-8 for long names, too.
  -- sjt

  [[ So, in the interest of getting XEmacs' font selection almost coherent,
  I'm passing back the short name, as a Lisp String, decoded from UTF-8. ]]

  I don't think this is close to coherency; even without the charset and
  lang properties fontconfig names are too unwieldy to use.  We need to
  rethink the approach here.  I think probably Lisp_Font_Instance.name should
  contain the font name as specified to Lisp (almost surely much shorter than
  shortname, even, and most likely wildcarded), while
  Lisp_Font_Instance.truename should contain the longname.  For now, I'm
