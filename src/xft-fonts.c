...
/* #### TO DO ####
   ...
   . Find out how fontconfig fontnames are encoded (ISO 8859/1, UTF-8?).
   . Implement FcConfigs.
*/
...
#ifdef THIS_ACTUALLY_WORKS_AFTER_ALL
static inline FcChar8 *extract_fcapi_string (Lisp_Object str);
...
/* 
   ...
   is a Lisp string.
*/
inline static FcChar8 *
extract_fcapi_string (Lisp_Object str)
{
  return (FcChar8 *) NEW_LISP_STRING_TO_EXTERNAL (str, Qnative);
}
      ...
      (pattern))
{
  ...
  /* #### fontconfig doesn't promise an encoding of names.
     What charset does an Xft font name use? */
  Lisp_Object res = build_ext_string (temp, Qx_font_name_encoding);
  free (temp);
  return res;
  }
}
      ...
      (pattern))
{
  ...
  /* #### What charset does an Xft/fontconfig font name use? */
  return res ? build_ext_string (temp, Qx_font_name_encoding) : Qnil;
}
  ...
  switch (fc_result)
	...
	case FcTypeString:
	  return ((!NILP (type) && !EQ (type, Qstring))
		  ...
		  : build_ext_string (fc_value.u.s, Qx_font_name_encoding));
	case FcTypeBool:
...
/* new functions in xft reloaded #3; where are they used? */
/* color conversion to xft colors */
/* #### move these to objects-x.c */
#define MINL(x,y) ((((unsigned long) (x)) < ((unsigned long) (y))) \
		   ? ((unsigned long) (x)) : ((unsigned long) (y)))

/* #### this should really take a struct device *, right? */
XftColor
xft_get_color (Display *dpy, Colormap cmap, Lisp_Object c, int dim)
{
  ...
  if (COLOR_INSTANCEP (c))
    return COLOR_INSTANCE_X_XFTCOLOR (XCOLOR_INSTANCE (c));
  else if (INTP (c))
